...
class TubeletEmbedding(layers.Layer):
    def __init__(self, embed_dim, tubelet_size, **kwargs):
        super().__init__(**kwargs)
        self.projection = layers.Conv3D(
            filters=embed_dim, kernel_size=tubelet_size, strides=tubelet_size, padding="VALID"
        )
    
    def call(self, videos):
        projected_patches = self.projection(videos)
        flattened_patches = layers.Reshape((-1, EMBED_DIM))(projected_patches)
...
def TransformerBlock(embed_dim, num_heads, mlp_dim, dropout_rate, block_name):
    ...
    return keras.Model(inputs, x, name=block_name)


def FactorizedEncoder(input_shape, num_classes, tubelet_size, embed_dim, num_heads, mlp_dim, num_layers, dropout_rate):
    inputs = layers.Input(shape=input_shape)
    
    # Tubelet Embedding
    x = TubeletEmbedding(embed_dim, tubelet_size)(inputs)
    
    # Positional Encoding
    x = PositionalEncoder(embed_dim)(x)

    # Spatial Transformer
    for i in range(num_layers):
        x = TransformerBlock(embed_dim, num_heads, mlp_dim, dropout_rate, f"spatial_transformer_block_{i}")(x)

    # Temporal Transformer
    x = layers.Reshape((-1, embed_dim))(x)
    for i in range(num_layers):
        x = TransformerBlock(embed_dim, num_heads, mlp_dim, dropout_rate, f"temporal_transformer_block_{i}")(x)

    # Global Average Pooling and Classification
    x = layers.GlobalAveragePooling1D()(x)
    outputs = layers.Dense(num_classes, activation="softmax")(x)

    return keras.Model(inputs=inputs, outputs=outputs, name="factorized_encoder")

class CastToFloat32(layers.Layer):
    def call(self, inputs):
        return tf.cast(inputs, tf.float32)

def spatial_transformer_encoder(inputs, num_layers, num_heads, embed_dim, layer_norm_eps):
    x = inputs
...
def temporal_transformer_encoder(inputs, num_layers, num_heads, embed_dim, layer_norm_eps):
    x = inputs
    for _ in range(num_layers):
        # Layer normalization 1
        x1 = layers.LayerNormalization(epsilon=layer_norm_eps)(x)
