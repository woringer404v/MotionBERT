...
def temporal_shift(batch_tensor, max_shift=5):
    """
    Apply temporal shift to a batch of pose data.
    
    :param batch_tensor: Input tensor of shape [batch_size, num_persons, num_frames, num_keypoints, num_channels].
    :param max_shift: Maximum number of frames to shift.
    :return: Temporally shifted tensor.
    """
    shifted_batch = []
    for sample in batch_tensor:
        shift = random.randint(-max_shift, max_shift)
        shifted_sample = torch.roll(sample, shifts=shift, dims=2)  # Shift along the frame dimension
...
def train_with_config(args, opts):
    ...
    if not opts.evaluate:
        #optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=0.1, momentum=0.9, weight_decay=0)
        ...
        for epoch in range(st, args.epochs):
            ...
            for idx, batch_data in enumerate(train_data_loaders[0]):    # (N, 2, T, 17, 3) to (N, 1, T, 17, 3136)
                ...
                batch_data['imgs'] = temporal_shift(batch_data['imgs'])  # Apply temporal shift
                batch_input = batch_data['imgs']
                batch_gt = batch_data['label']
